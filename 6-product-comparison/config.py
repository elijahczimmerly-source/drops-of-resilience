...
# Task folder still holds local `data/` in many setups; override with env on the pipeline if needed.
_DATA_LEGACY = REPO_ROOT / "4-test8-v2-pr-intensity" / "data"
GEO_MASK = Path(
    os.environ.get("DOR_TEST8_GEO_MASK_NPY", str(_DATA_LEGACY / "geo_mask.npy"))
)

GRIDMET_TARGETS = Path(
    os.environ.get(
        "DOR_TEST8_GRIDMET_TARGETS_DAT",
        str(_DATA_LEGACY / "gridmet_targets_19810101-20141231.dat"),
    )
)
