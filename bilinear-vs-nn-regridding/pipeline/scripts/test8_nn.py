...
OUT_DIR = r"C:\drops-of-resilience\bilinear-vs-nn-regridding\pipeline\output\nearest_neighbor"
...
def process_variable(var_name):
    ...
    if os.path.exists(F_INPUTS_EARLY):
        ...
        np.savez_compressed(os.path.join(OUT_DIR, f"Stochastic_NN_{var_name}_1850_1980.npz"),
    ...
    if os.path.exists(F_INPUTS_FUTURE):
        ...
        np.savez_compressed(os.path.join(OUT_DIR, f"Stochastic_NN_{var_name}_SSP585_2015_2100.npz"),
...
if __name__ == "__main__":
    ...
    df1.to_csv(os.path.join(OUT_DIR, "NN_Table1_Pooled_Metrics.csv"), index=False)
    ...
    df2.to_csv(os.path.join(OUT_DIR, "NN_Table2_PerCell_Metrics.csv"), index=False)

    # Save outputs
    log("Saving main-period outputs...")
    for i, var in enumerate(VARS_INTERNAL):
        np.savez_compressed(os.path.join(OUT_DIR, f"Stochastic_NN_{var}.npz"),
                            data=full_sim[:, i], dates=DATES_ALL.values)
        log(f"  Saved {var}")

    del full_sim; gc.collect()
    log(f"\nNearest-neighbor path complete. Total time: {(time.time() - _T0) / 60:.2f} mins.")
