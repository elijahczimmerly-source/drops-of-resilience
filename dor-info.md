...
| **`6-product-comparison` and regridding** | **[`run_benchmark.py`](6-product-comparison/scripts/run_benchmark.py)** does **not** call [`regrid_to_gridmet.py`](#key-scripts) or [`pipeline/scripts/regrid/`](pipeline/scripts/regrid/). It reads **DOR** from existing `Stochastic_V8_Hybrid_*.npz` (built when you ran the downscaler on memmaps whose **GCM inputs** were produced earlier by server/local regrid). For the default **`dor_native`** suite, it reads **LOCA2** / **NEX** NetCDFs and **interpolates** them onto the Iowa GridMET grid in code ([`load_loca2.py`](6-product-comparison/scripts/load_loca2.py), [`load_nex.py`](6-product-comparison/scripts/load_nex.py)); **`loca2_native`** / **`nex_native`** use native-grid loaders instead. **Obs** = GridMET targets memmap on **`dor_native`**; native suites interpolate obs to the evaluation mesh as implemented in [`benchmark_io.py`](6-product-comparison/scripts/benchmark_io.py). |
| **`6-product-comparison` suite roots & `DOR_BENCHMARK_SUITE`** | Benchmark and figure outputs are **not** under a single shared `output/` folder. There are **three parallel suite directories** next to [`6-product-comparison/README.md`](6-product-comparison/README.md): **`dor_native/`** (default) — GridMET 216×192 evaluation mesh; **`loca2_native/`** — LOCA2 native grid; **`nex_native/`** — NEX 0.25° native grid. Set **`DOR_BENCHMARK_SUITE=dor_native|gridmet_4km|loca2_native|nex_native`** (default **`dor_native`**). The legacy token **`gridmet_4km`** is still accepted and is **normalized to `dor_native`** in code ([`scripts/grid_suites.py`](6-product-comparison/scripts/grid_suites.py)). Full table: [`6-product-comparison/SUITES.md`](6-product-comparison/SUITES.md). Multi-panel plots (hist / validation / climate Δ) live **directly under** each suite’s **`figures/`** (no `4km_plots/` or `figures/plots/` segment). Secondary PNGs use separate subfolders (e.g. **`benchmark_figures/`**, **`validation_obs_vs_dor/`**, **`nex_rsds_diagnostic/`**) so the main `figures/` tree stays the comparison-driver gallery. |
| **What the product benchmark does *not* measure** | It compares **2006–2014 simulated vs GridMET** (KGE, RMSE, Ext99, etc.) on the evaluation grid for the chosen suite. It does **not** assess preservation of a **climate change signal** (e.g. future minus historical vs other products); that would require a separate analysis with SSP outputs. |
| **LOCA2 vs NEX native grids** | Archive resolutions are roughly **LOCA2 ~6 km** (≈1/16°) and **NEX-GDDP-CMIP6 0.25° × 0.25°** (~25–28 km at Iowa). See [`6-product-comparison/LITERATURE.md`](6-product-comparison/LITERATURE.md). With **`DOR_BENCHMARK_SUITE=dor_native`**, LOCA2 and NEX are **interpolated to the Iowa 216×192 GridMET grid** for comparison. With **`loca2_native`** or **`nex_native`**, evaluation uses that product’s **native** mesh (DOR/S3/obs interpolated as needed); see [`WORKLOG_NATIVE_RESOLUTION.md`](6-product-comparison/WORKLOG_NATIVE_RESOLUTION.md). |
...
**Cross-product benchmark:** **[`6-product-comparison/`](6-product-comparison/)** compares this DOR output (blend **0.65**) to **LOCA2** and **NEX-GDDP-CMIP6** on **MPI**, using **`DOR_BENCHMARK_SUITE`** to select the evaluation grid (default **`dor_native`** = Iowa **GridMET** memmap and **2006–2014** pooled metrics on that mesh). Artifacts land under **`dor_native/`**, **`loca2_native/`**, or **`nex_native/`** per suite — see [`SUITES.md`](6-product-comparison/SUITES.md). See **External product comparison** later in this file.
...
- **External verification** — compare against other organizations' pipeline outputs. **LOCA2 and NEX-GDDP-CMIP6** are on the server under `Spatial_Downscaling\Downscaled_Products\`. A quantitative benchmark for **MPI** is implemented locally in [`6-product-comparison/`](6-product-comparison/) — default **`dor_native`** (GridMET-target evaluation), plus native-grid suites under **`loca2_native/`** and **`nex_native/`** ([`SUITES.md`](6-product-comparison/SUITES.md)); see **External product comparison** in this file.
...
**Purpose:** Compare **our** downscaled product to **published statistical downscaling archives** using a **common validation truth** appropriate to the selected **benchmark suite**, without claiming the external products were trained to GridMET.

**Suite model (current layout):** All benchmark artifacts for the GridMET-target stack live under **`6-product-comparison/dor_native/`** (not a shared `output/` folder). Parallel **native-grid** suites use **`6-product-comparison/loca2_native/`** and **`6-product-comparison/nex_native/`**. Select with **`DOR_BENCHMARK_SUITE`** (default **`dor_native`**; legacy **`gridmet_4km`** is normalized to **`dor_native`**). Canonical suite ids and folder map: [`6-product-comparison/SUITES.md`](6-product-comparison/SUITES.md). Routing and aliases: [`6-product-comparison/scripts/grid_suites.py`](6-product-comparison/scripts/grid_suites.py). Central paths: [`6-product-comparison/config.py`](6-product-comparison/config.py) (`DOR_NATIVE_ROOT`, `OUTPUT_DIR` = `dor_native`, `FIG_DIR` / `FIG_4KM_PLOTS` = flat `dor_native/figures/`).

**Scope:** Historical **validation-period** metrics only (config default **2006–2014**). This folder does **not** run the OTBC **regrid** pipeline; it consumes **already-built** DOR NPZs and aligns external products per suite. For **`dor_native`**, LOCA2 and NEX are **interpolated** to the Iowa **216×192** GridMET mesh. For **`loca2_native`** / **`nex_native`**, loaders evaluate on each product’s **native** grid (see **Clarifications** and [`WORKLOG_NATIVE_RESOLUTION.md`](6-product-comparison/WORKLOG_NATIVE_RESOLUTION.md)). For **climate-change signal** (e.g. SSP minus historical), use the climate-signal scripts in this package (e.g. `run_climate_signal_stages.py`) or tooling that compares future stacks to historical means — not the validation-only CSVs alone.
...
**Alignment (`dor_native` suite):** External fields are **linearly interpolated** in lon/lat (xarray) onto the **GridMET lat/lon** from `Data\Cropped_Iowa\GridMET\Cropped_pr_2006.npz`. **pr** from LOCA2/NEX converted **kg m⁻² s⁻¹ → mm/day** (×86400). Days aligned by **calendar date** (NEX timestamps normalized). Native suites use different regridding paths in [`benchmark_io.py`](6-product-comparison/scripts/benchmark_io.py).

**Repo layout:** [`6-product-comparison/README.md`](6-product-comparison/README.md), [`SUITES.md`](6-product-comparison/SUITES.md), [`WORKLOG.md`](6-product-comparison/WORKLOG.md) (audit trail + NEX-`rsds` diagnosis), [`WORKLOG_NATIVE_RESOLUTION.md`](6-product-comparison/WORKLOG_NATIVE_RESOLUTION.md) (native suites + layout history), [`LITERATURE.md`](6-product-comparison/LITERATURE.md). **Run:** `conda activate drops-of-resilience` then e.g. `python 6-product-comparison/scripts/run_benchmark.py` (long run if LOCA2 is read over the network). End-to-end: [`6-product-comparison/scripts/run_e2e_suite.py`](6-product-comparison/scripts/run_e2e_suite.py) (env **`DOR_E2E_SUITES`**).

**Outputs (by suite root):** Wide benchmark CSVs and climate-signal CSVs sit at the suite root (e.g. **`dor_native/benchmark_summary_<pipeline_id>.csv`**). **Multi-panel** comparison figures (**hist** / **validation** / **delta**) are under **`<suite>/figures/`** directly (subfolders such as `hist_1981_2014/`, `validation_2006_2014/`, `delta_future_minus_hist/`, plus `index.html`). **Secondary** PNG outputs use dedicated subfolders: **`benchmark_figures/`** (KGE / Ext99 bars from `run_benchmark.py`), **`validation_obs_vs_dor/`** (validation-era side-by-sides and `validation_ts_*.png` from `plot_validation_period.py`), **`nex_rsds_diagnostic/`** (PNG diagnostics from `diagnose_nex_rsds.py`; tabular outputs stay at suite root). Provenance mirrors: **`benchmark_bundle/`** (see `collect_benchmark_provenance.py`).
...
**Takeaway:** For **`dor_native`**, **GridMET** is the **evaluation** reference, not the **training** reference for NEX or LOCA2. A large **mean bias** can still appear where the **external analysis** disagrees systematically with GridMET (especially **shortwave**).
...
Full metric table: [`dor_native/benchmark_summary.csv`](6-product-comparison/dor_native/benchmark_summary.csv) and/or per-pipeline files such as [`benchmark_summary_test8_v4.csv`](6-product-comparison/dor_native/benchmark_summary_test8_v4.csv) (exact names depend on `DOR_PIPELINE_ID` / suite). Detailed weakness inventory in [`6-product-comparison/dor-weaknesses.md`](6-product-comparison/dor-weaknesses.md) (if that file still cites legacy `output/` paths, prefer **`dor_native/`** for the GridMET mesh suite).
...
Spatial validation maps generated for all 6 vars: single-day snapshots (5 dates), time-mean (2006–2014), and seasonal-mean (DJF/MAM/JJA/SON). Under the current layout, GridMET-vs-DOR side-by-sides from `plot_validation_period.py` live under **`6-product-comparison/dor_native/validation_obs_vs_dor/`** (`individual days/`, `time aggregated/`). Older runs may still show paths under `output/figures/dor side-by-side/` — same role, new location.
...
1. **Native LOCA2 / NEX resolution benchmarks** — **Implemented in repo** (`DOR_BENCHMARK_SUITE=loca2_native|nex_native`, outputs under [`loca2_native/`](6-product-comparison/loca2_native/) and [`nex_native/`](6-product-comparison/nex_native/); default GridMET-target suite remains **`dor_native`**). Remaining work is **science / interpretation** (comparing native vs `dor_native` stories, documentation polish), not missing plumbing. Entry points: [`run_benchmark.py`](6-product-comparison/scripts/run_benchmark.py), [`benchmark_io.py`](6-product-comparison/scripts/benchmark_io.py). **Resolved elsewhere:** **pr WDF** tuning ([`8-WDF-overprediction-fix/`](8-WDF-overprediction-fix/)) — `PR_WDF_THRESHOLD_FACTOR` **1.65** default on **`test8_v4`**; see `output/wdf_216x192_confirmation.md`.
